...
class TemperatureScheduler:
    """
    Manages temperature scheduling for LLM generation to balance exploration and exploitation.
    
    High temperature (e.g., 0.8-1.0): More randomness, diversity, exploration
    Low temperature (e.g., 0.0-0.3): More focused, deterministic, exploitation
    """
    
    def __init__(self, schedule_type: str = "cosine", initial_temp: float = 0.8, 
                 final_temp: float = 0.0, total_steps: int = 10):
        """
        ...
        logger.info(f"[TEMPERATURE-SCHEDULER] Initialized {schedule_type} schedule: {initial_temp:.2f} → {final_temp:.2f} over {total_steps} steps")
    
    def get_temperature(self, step: int = None) -> float:
        """Get temperature for the current or specified step."""
        if step is None:
        ...
        if self.schedule_type == "linear":
            # Linear decay: T = T_init - (T_init - T_final) * progress
            temp = self.initial_temp - (self.initial_temp - self.final_temp) * progress
        
        elif self.schedule_type == "exponential":
            # Exponential decay: T = T_final + (T_init - T_final) * exp(-5 * progress)
            import math
            temp = self.final_temp + (self.initial_temp - self.final_temp) * math.exp(-5 * progress)
        
        elif self.schedule_type == "cosine":
            # Cosine annealing: smooth decay with cosine curve
            import math
            temp = self.final_temp + 0.5 * (self.initial_temp - self.final_temp) * (1 + math.cos(math.pi * progress))
        
        elif self.schedule_type == "step":
            # Step decay: constant for first half, then drop to final
            if progress < 0.5:
            ...
            # Adaptive: Higher temperature early and late (for exploration), lower in middle (for refinement)
            import math
            # U-shaped curve: high at start and end, low in middle
            temp = self.final_temp + (self.initial_temp - self.final_temp) * (1 - 2 * abs(progress - 0.5))
...
def generate_initial_solution_with_temperature_scheduling(problem_statement: str, code_skeleton: str) -> str:
    """
    Generate initial solution with temperature scheduling for controlled diversity.
    
    Temperature scheduling strategy:
    - Start with HIGH temperature (0.8) for diverse initial solutions
    - Gradually decrease to LOW temperature (0.0) for refined solutions
    - Use cosine annealing for smooth transition
    """
    NUM_SOLUTIONS = 5
    ...
    # Initialize temperature scheduler
    # Cosine schedule provides smooth decay with good exploration-exploitation balance
    scheduler = TemperatureScheduler(
        schedule_type="cosine",
        initial_temp=0.8,  # High temperature for initial diversity
        final_temp=0.0,    # Low temperature for final refinement
        total_steps=NUM_SOLUTIONS
    )
    
    logger.info(f"[TEMP-SCHEDULE] Generating {NUM_SOLUTIONS} solutions with temperature scheduling")
    
    solutions = []
    temperatures_used = []
    retry = 0
    
    while len(solutions) < NUM_SOLUTIONS and retry < 15:
        try:
            # Get temperature for current solution
            temperature = scheduler.step()
            temperatures_used.append(temperature)
            
            logger.info(f"[TEMP-SCHEDULE] Generating solution {len(solutions) + 1}/{NUM_SOLUTIONS} with temperature={temperature:.3f}")
            
            # Alternate between models for additional diversity
            model_index = len(solutions) % len(models)
            current_model = models[model_index]
            
            # Try multi-step reasoning first with scheduled temperature
            solution = generate_solution_with_multi_step_reasoning_temp(
                problem_statement, code_skeleton, temperature
            )
            
            if solution:
                solutions.append(solution)
                logger.info(f"[TEMP-SCHEDULE] Generated solution {len(solutions)} (temp={temperature:.3f})")
            else:
                # Fallback to single-step approach with temperature
                messages = [
                    {
                        "role": "system",
                ...
                response = EnhancedNetwork.make_request(messages, model=current_model, temperature=temperature)
                ...
                if solution:
                    solutions.append(solution)
                    logger.info(f"[TEMP-SCHEDULE] Generated solution {len(solutions)} using fallback (temp={temperature:.3f})")
    ...
    if not solutions:
        logger.error("[TEMP-SCHEDULE] Failed to generate any solutions")
        return ""
    
    # Log temperature schedule summary
    logger.info(f"[TEMP-SCHEDULE] Temperature schedule used: {[f'{t:.3f}' for t in temperatures_used]}")
    logger.info(f"[TEMP-SCHEDULE] Average temperature: {sum(temperatures_used)/len(temperatures_used):.3f}")
    
    # If we have only one solution, return it
    if len(solutions) == 1:
        logger.info("[TEMP-SCHEDULE] Only one solution generated, returning it")
        return solutions[0]
    
    # Use LLM to choose the best solution (with low temperature for focused selection)
    logger.info(f"[TEMP-SCHEDULE] Evaluating {len(solutions)} solutions with temperature=0.0")
    ...
    for i, solution in enumerate(solutions):
        ...
        analysis = analyze_missing_edge_cases(solution, problem_statement)
        
        solution_validations.append(validation)
        solution_analyses.append(analysis)
        
        logger.info(f"Solution {i+1} (temp={temperatures_used[i]:.3f}) - Quality: {validation['comment_quality']}, Risk: {analysis['risk_assessment']}")
    ...
    Below are {len(solutions)} different solutions generated with varying temperatures (higher temp = more creative, lower temp = more focused).
    
    Please analyze each solution and select the best one based on:
    ...
    5. Presence and quality of edge case comments
    6. Completeness of edge case coverage

    Edge Case Analysis:
    """
    for i, (solution, validation, analysis, temp) in enumerate(zip(solutions, solution_validations, solution_analyses, temperatures_used), 1):
        comparison_prompt += f"""Solution {i} (temperature={temp:.3f}): 
    ...
    for i, solution in enumerate(solutions, 1):
        comparison_prompt += f"=== SOLUTION {i} (temp={temperatures_used[i-1]:.3f}) ===\n{solution}\n\n"
    
    comparison_prompt += "\nRespond with: BEST_SOLUTION: <number>"
    ...
    try:
        ...
        # Use temperature=0.0 for deterministic selection
        response = EnhancedNetwork.make_request(comparison_messages, model=models[0], temperature=0.0)
        ...
        if best_solution_match:
            selected_index = int(best_solution_match.group(1)) - 1
            if 0 <= selected_index < len(solutions):
                logger.info(f"[TEMP-SCHEDULE] Selected solution {selected_index + 1} (generated with temp={temperatures_used[selected_index]:.3f})")
                return solutions[selected_index]
            else:
                logger.warning(f"Invalid solution index, returning first solution")
                return solutions[0]
        else:
...
def generate_solution_with_multi_step_reasoning_temp(problem_statement: str, code_skeleton: str, temperature: float) -> str:
    """Generate solution with multi-step reasoning using specified temperature."""
    retry = 0
    ...
    while retry < 3:  # Reduced retries for temperature-scheduled generation
        try:
            code_response = EnhancedNetwork.make_request(code_generation_messages, model=QWEN_MODEL_NAME, temperature=temperature)
            logger.info(f"Step 1 - Code Generation completed (temp={temperature:.3f})")
            ...
            # Use low temperature for loop checking (deterministic)
            ...
            logger.info(f"Solution generation completed with temperature={temperature:.3f}")
            return solution
        except Exception as e:
            retry += 1
            logger.warning(f"Exception in generate_solution_with_multi_step_reasoning_temp: {e}")
            time.sleep(1)
...
def process_create_task(input_dict):
    ...
    # Use temperature scheduling for controlled diversity in solution generation
    logger.info("[TEMP-SCHEDULE] Using temperature scheduling for solution generation")
    initial_solution = generate_initial_solution_with_temperature_scheduling(problem_statement, code_skeleton)
